...
def printgraphs(graph,type):

   file=open(GRAPH_PATH+"%s_%s.js"%(graph,type),'r')
   greaphdata=file.read()
   if type[len(type)-6:len(type)]!=='cpl.js':
	...
	print """<div class='graphs'>
      <div id="%s" style="width: 900px; height: 600px;"></div>
        ...
        </script> <script type="text/javascript">"""%(graph)
	print graphdata
