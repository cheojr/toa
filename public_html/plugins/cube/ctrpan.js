...
var srcIp = "",dstIp = "",src_pre = "0",dst_pre = "0",time="",inp_date,uid,sid,remote ; // Stores the user input for do the parsing.
...
function getUrl(request_type){
	...
	if (request_type==0){
		return 'index.cgi?fromtoa='+inp_date+'&PARSE=1'+'&uid='+uid+'&sid='+sid+'&remote='+remote;
	}

	else if (request_type==1){
		return  'index.cgi?srcIp='+srcIp+'&uid='+uid+'&sid='+sid+'&remote='+remote+'&dstIp='+dstIp+'&src_pre='+src_pre+'&dst_pre='+dst_pre+'&time='+time+'&PARSE=1'+'&date='+inp_date+"&thold="+document.getElementById('opt').value+"&tvalue="+document.getElementById('threshold_size').value+'|'+document.getElementById('unit').value;
