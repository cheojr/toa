...
class runpage:
	...
	def __init__(self):
	    ...
	    self.createiframe()#creates the iframe for the graphs and calls the necessary functions to create it
    
	def createiframe(self):
                
                file=open('/var/www/html/graphs/p2p_graph.html','w')
	...
	#################################Finishes writing the root and its adjacencies
                ...
                for i in range (len(From)):
                    ...
                    file.write ((""" ],
                            ...
                            """) %(colors[2],all_octects,all_packets,all_flows,From[i],GetLabel(c,From[i]))) #Closes the adjacencies arra and specifies the color,shape and dim that the source network is going to have in the viz
